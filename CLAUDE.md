...
2. **`uuid` 9.0.1 → 11.1.1** — verified compatible (CJS `main: ./dist/cjs/index.js`, `??` and `?.` syntax supported by Electron 11 Chrome 87, no babel transpile of node_modules needed because webpack-production doesn't minify). Pending — apply when convenient. Do NOT bump past 12.x.
3. **Mermaid tier B / C** (`~9.3.0` then `^9.4.3`) — escalate only after manually rendering one of each diagram type from a packaged build and confirming no `ChunkLoadError` from the v9.2+ lazy-load chunks.
